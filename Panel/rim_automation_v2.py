...
# GOOGLE MASTER + GOOGLE REPORT VERSION
# ============================================================
...
def run_step1(process_date, selected_cities):
    ...
    output_dir = os.path.join(OUTPUT_FOLDER,folder_date,"step1_mapping")
...
def run_step2(process_date):
    ...
    input_dir = os.path.join(OUTPUT_FOLDER, folder_date, "step1_mapping")
    output_dir = os.path.join(OUTPUT_FOLDER, folder_date, "step2_rimweight")

    os.makedirs(output_dir, exist_ok=True)

    files = [f for f in os.listdir(input_dir) if f.endswith(".csv") and "summary" not in f.lower()]
...
def run_step3(process_date):
    ...
    if not files:
        raise Exception("No Step-2 files found")
...
# ============================================================
# STEP 3 – FULL VALIDATION (CITY + GENDER + AGE BENCHMARKS)
# ============================================================
...
if __name__ == "__main__":
    ...
    print("1. Enter Processing Date")
    print("2. Process D-1 Date")
    ...
    if option == "1":
        ...
        process_date = input(
            "\nEnter Processing Date (DD-MM-YYYY): "
        ).strip()
        ...
        print(
            f"\nD-1 Date Selected : {process_date}"
        )
    ...
    if os.path.exists(process_output_folder):

        print("\n" + "=" * 60)
        print(f"Output folder already exists:")
        print(process_output_folder)
        print("=" * 60)

        reprocess = input(
            "\nDo you want to Re-Process this date? (Yes/No): "
        ).strip().lower()

        if reprocess not in ["yes", "y"]:
